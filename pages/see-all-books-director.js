...
export const getServerSideProps = async (context) => {
  try {
    const result = await mysql.query(
      "SELECT * FROM requestform WHERE sendtoDirector = 1 and approvalDirector = 0 "
    );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{values.verified ? "Yes" : "No"}</div>
        ),
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{values.signatureAcquisition ? "Signed" : "No"}</div>
        ),
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{values.signatureDean ? "Signed" : "No"}</div>
        ),
      },
