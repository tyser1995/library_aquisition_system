...
export const getServerSideProps = async (context) => {
  try {
    ...
    const result = await mysql.query(
      "SELECT * FROM requestform WHERE approvalFinancePayment = 1 AND approvalDirector = 1 AND approvalVpaaPayment = 1 AND approvalVpaa = 1 AND status = 0 "
    );
  ...
  const postRequestedBooks = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{values.approvalPresident ? "Yes" : "No"}</div>
        ),
      },
      {
