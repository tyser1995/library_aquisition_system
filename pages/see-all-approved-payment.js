...
export const getServerSideProps = async (context) => {
  try {
    ...
    const result = await mysql.query(
      `SELECT * FROM requestform WHERE approvalFinancePayment = 1  `
    );
  ...
  const postRequestedBooks = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{values.approvalFinancePayment ? "Yes" : "No"}</div>
        ),
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{new Date(values.approvalFinanceDate).toDateString()}</div>
        ),
      },
    ],
    []
