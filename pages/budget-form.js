...
const BudgetForm = ({ selectDepartment }) => {
  ...
  const handleSelectBudget = async (values) => {
    ...
    const deduct_budget = (parseInt(document.getElementById('totalSubtracted').value) + values.budget);
    const budget_subtracted = document.getElementById('totalSubtracted').value.replace('-', '');
    const updateBudget =  await axios.post('/api/borrowedbudget/updateDepartmentBudget', {
      budget: deduct_budget,
      selectDepartment: values.selectDepartment,
      previousBudget: values.budget,
      });

    const insertBudget = await axios.post('/api/borrowedbudget/insertDepartment', {
        budget: parseInt(budget_subtracted) + parseInt(document.getElementById('totalBudget').value),
                    ...
                    { data.subtracted < 1 || data.subtracted == 0 ? <>
                        <div className="">
                         ...
                         px-6 py-3 rounded shadow-md "
                            type="button"
                            onClick={toggleDept}
                            style={{ display: data.subtracted < 1 ? 'none': '' }}
                          >
