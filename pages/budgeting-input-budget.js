...
export const getServerSideProps = async () => {
  ...
  const totalAddedBudget = await mysql.query(
    'SELECT sum(budget) FROM add_budget WHERE selectDepartment NOT IN ("Filipiniana","Reference") and YEAR(dateAdded) = YEAR(CURDATE())'
  );
  const totalAddedRefFil = await mysql.query(
    'SELECT sum(budget) FROM add_budget WHERE selectDepartment  IN ("Filipiniana","Reference") and YEAR(dateAdded) = YEAR(CURDATE())'
  );
...
export default function SignIn({
  ...
  const [txtTotalAddedBud, setTotalAddedBud] = useState(
    totalAddedBud.split(":")[1].toString().replace("}]", "")
  );
  const [txtTotalAddedRefFil, setTotalAddedBudRefFil] = useState(
    totalPostAddedRefFil.split(":")[1].toString().replace("}]", "")
  );
  ...
  const handleKeyPressEnrollees = (event) => {
    ...
    const inputTxtBoxEnrollees =
      document.getElementById("textboxEnrollees").value;
    let totalBud = document.getElementById("budget").value;

    if (inputTxtBoxLibFee != "NaN" || inputTxtBoxLibFee != "") {
      totalBud =
        document.getElementById("textboxEnrollees").value *
        document.getElementById("textboxLibFee").value;
  ...
  const handleKeyPress = (event) => {
    ...
    let totalBud = document.getElementById("budget").value;
    if (
      document.getElementById("textboxEnrollees").value == "NaN" ||
      document.getElementById("textboxLibFee").value == "NaN"
    ) {
    }
    totalBud =
      document.getElementById("textboxEnrollees").value *
  ...
  const postCost = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "action",
        Cell: ({ row: { values } }) => (
          <BudgetForm selectDepartment={values.selectDepartment} />
        ),
      },
    ],
    []
        ...
        <Form
          ...
          render={({ handleSubmit }) => (
            <form
              ...
              className=" px-8 pt-8 pb-8 bg-white rounded-md my- mx-auto h-auto w-full shadow-lg  "
            >
              {/* MODAL ENDS HERE */}
              <Field
              ...
              <div className="grid grid-cols-3  gap-4 mt-4">
                <div className="">
                  <label htmlFor="selectDepartment" className="block p">
                    ...
                    <Field
                      ...
                      component="select"
                      className="rounded-md  text-xs divide-y divide-dashed text-gray-500 border-gray-300 space -space-y-1 w-full mt-1"
                      required
                    >
                      ...
                      <option
                        className="text-xs  text-gray-500"
                        value="College of Business & Accountancy"
                      >
                      ...
                      <option
                        className="text-x text-gray-500"
                      ...
                      <option
                        className="text-xs  text-gray-500"
                        value="College of Medical Laboratory Science"
                      >
                      ...
                      <option className="text-xs text-gray-500" value="Kinder">
                        Kinder
                      </option>
                      <option
                        className="text-xs text-gray-500"
                ...
                <div className="col-start-2">
                  <label htmlFor="budget" className="block pb-2">
                    <span className="block text-xs  pt-2 text-gray-500 ">
                      Total Budget
                    </span>
                    <Field
