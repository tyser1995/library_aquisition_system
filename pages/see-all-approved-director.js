...
export const getServerSideProps = async (context) => {
  try {
    ...
    const result = await mysql.query(
      `SELECT * FROM requestform WHERE approvalDirector = 1  `
    );
  ...
  const postRequestedBooks = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row: { values } }) => (
          <div>{new Date(values.approvalDateDirector).toDateString()}</div>
        ),
      },
    ],
    []
